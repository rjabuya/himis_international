...
import com.kajabuyahmis.bean.lab.PatientInvestigationController;
import com.kajabuyahmis.bean.pharmacy.PharmacyPreSettleController;
import com.kajabuyahmis.data.BillClassType;
import com.kajabuyahmis.data.BillNumberSuffix;
import com.kajabuyahmis.data.BillSummery;
import com.kajabuyahmis.data.BillType;
import com.kajabuyahmis.data.FeeType;
import com.kajabuyahmis.data.HistoryType;
...
import com.kajabuyahmis.ejb.EjbApplication;
import com.kajabuyahmis.ejb.PharmacyBean;
import com.kajabuyahmis.ejb.StaffBean;
import com.kajabuyahmis.entity.AgentHistory;
...
import com.kajabuyahmis.facade.AgentHistoryFacade;
...
import com.kajabuyahmis.facade.EmailFacade;
import com.kajabuyahmis.facade.ItemBatchFacade;
...
public class BillSearch implements Serializable {
    ...
    public void fillBillTypeSummery() {
        ...
        if (billClassType == null) {
            j = "select new com.kajabuyahmis.data.BillSummery(b.paymentMethod, sum(b.total), sum(b.discount), sum(b.netTotal), sum(b.vat), count(b), b.billType) "
                    + " from Bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        } else {
            j = "select new com.kajabuyahmis.data.BillSummery(b.paymentMethod, b.billClassType, sum(b.total), sum(b.discount), sum(b.netTotal), sum(b.vat), count(b), b.billType) "
                    + " from Bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        }
    ...
    public void fillBillFeeTypeSummery() {
        ...
        String j;
        if (billClassType == null) {
            j = "select new com.kajabuyahmis.data.BillSummery(b.paymentMethod, sum(bf.feeGrossValue), sum(bf.feeDiscount), sum(bf.feeValue), sum(bf.feeVat), count(b), b.billType) "
                    + " from BillFee bf inner join bf.bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        } else {
            j = "select new com.kajabuyahmis.data.BillSummery(b.paymentMethod, b.billClassType, sum(bf.feeGrossValue), sum(bf.feeDiscount), sum(bf.feeValue), sum(bf.feeVat), count(b), b.billType) "
                    + " from BillFee bf inner join bf.bill b "
                    + " where b.retired=false "
                    + " and b.billTime between :fd and :td ";
        }
